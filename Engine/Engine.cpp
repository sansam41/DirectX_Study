...
void Engine::ResizeWindow(int32 width,int32 height) {
	...
	// ::은 global namespace에서 찾아 사용하겠다라는 의미. 명시적으로 붙이는게 좋다.
	::AdjustWindowRect(&rect, WS_OVERLAPPEDWINDOW, false); // 윈도우 크기 조절
	::SetWindowPos(_window.hwnd, 0, 100, 100, width, height, 0); //윈도우 위치와 크기 세팅
...
void Engine::ShowFps() {
	...
	WCHAR text[100] = L"";
	::wsprintf(text, L"FPS : %d", fps);		// 포멧을 지정해 문자열 생성

	::SetWindowText(_window.hwnd, text);	// 화면에 출력
}
