...
// 점 하나만 넘겨줘서 현재 그려줄 대상인지 판단 후 사각형을 그려준다.
[maxvertexcount(6)]
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
    ...
    GroupMemoryBarrierWithGroupSync();  // 쓰레드 동기화
    ...
    if (g_particle[threadIndex.x].alive == 0)
    {
        while (true)
        {
            ...
            int originalValue;
            InterlockedCompareExchange(g_shared[0].addCount, expected, desired, originalValue); // 선착순 실행
